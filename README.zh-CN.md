...
## hall 工作流
- 从一条任务开始，agent 在同一条共享线程里协作——像一个所有人都能看到全部消息的群聊。
- `@` 单个 agent 路由到它；`@` 多个并行派发；不 `@` 任何人时新任务默认派给 planner / manager 角色的参与者，跟进消息默认派给 `main` agent。
- 想给整个 hall 统一协作风格可以改仓库根目录的 `HALL.md`，它会注入到 agent 的 setup prompt 里作为人格引导，但不会覆盖 mention 路由和反循环策略。
- 用 `安排后续顺序` 决定谁先做、后面谁接、每一步交接给谁（保存顺序 **不会** 自动开始执行；队列排好后结论卡会出现 `开始执行（...）`）。
- 进入执行后，每个人只做自己这一棒，并在同一条线程里显式 `@` 下一位交棒。
- 任务卡线程空闲超过 10 分钟没有 operator 输入会显示 `需要人类审核` 标签——你检查完点 `标记为已审核` 即可清掉。

> 架构细节——黑板、信箱 + 调度器、反循环策略链、observer 轮——见 [docs/HALL_ARCHITECTURE.md](./docs/HALL_ARCHITECTURE.md)。回复 / typing / 状态来源的契约（防退回归）见 [docs/HALL_REPLY_LIFECYCLE.md](./docs/HALL_REPLY_LIFECYCLE.md)。
